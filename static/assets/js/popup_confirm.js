function popupConfirmFun(id,message,description, link){
    ...
    popup.innerHTML = `
        ...
        <div class="absolute left-[50%] top-[50%] translate-y-[-50%] translate-x-[-50%] card bg-white w-96 bg-neutral text-neutral-content">

          <div class="card-body items-center text-center">
            <h2 class="card-title text-slate-900 text-lg">${message}</h2>
            <p class="text-slate-900 text-xs">${description}</p>
            <span class="img_preview"></span>
            <div class="card-actions justify-end">
              <button data-id="${id}" class="confirm btn btn-primary">Accept</button>
              <button class="cancle btn btn-ghost hover:text-slate-700">Deny</button>
            </div>
          </div>
        </div>
